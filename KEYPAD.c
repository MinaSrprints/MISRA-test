...
{
    DIO_vidWritePortDirection(KPD_COL_PORT,KPD_COL_MASK,APS_ZERO);
}

void KPD_ROW_INIT(void)
{
    DIO_vidWritePortDirection(KPD_ROW_PORT,KPD_ROW_MASK,APS_ONE);
    DIO_vidWritePortData(KPD_ROW_PORT,KPD_ROW_MASK,APS_ZERO);
}

void KPD_ROW_WRITE(unsigned char DATA)
{
    DATA = DATA << KPD_ROW_PIN_NUM;
    DIO_vidWritePortData(KPD_ROW_PORT,KPD_ROW_MASK,DATA);
}
